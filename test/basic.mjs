...
test('invalid host fails clearly',()=>{assert.throws(()=>delegate(null,'target'),/proto must be an object or function/);assert.throws(()=>delegate({},123),/property names must be strings or symbols/)})
...
test('CJS exports callable delegate',()=>{const require=createRequire(import.meta.url);const cjs=require('../dist/index.cjs');assert.equal(typeof cjs,'function');assert.equal(cjs,cjs.default);assert.equal(typeof cjs.Delegator,'function')})
