...
function runAssignmentCompatibility(delegate) {
  ...
  const inheritedSetterCalls = typeof host.inheritedWrites
...
test('modern auto(): writable function properties remain callable', () => {
  const delegate = modern
  ...
  const result = build(delegate)

  assert.equal(result.returnValue, 2)
  assert.equal(result.descriptor?.hasGet, true)
  assert.equal(result.descriptor?.hasSet, true)
  assert.ok((result.setterCount ?? 0) > 0)
})

test('modern additions: symbol keys and delegate.auto()', () => {
